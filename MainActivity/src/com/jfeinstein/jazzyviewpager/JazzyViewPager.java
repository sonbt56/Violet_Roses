...
public class JazzyViewPager extends ViewPager {
    ...
    public JazzyViewPager(Context context, AttributeSet attrs) {
        ...
        setFadeEnabled(ta.getBoolean(R.styleable.JazzyViewPager_fade_enabled, false));
    ...
    private void animateAccordion(View left, View right, float positionOffset,
        ...
        if (mState != State.IDLE) {
            ...
            if (left != null) {
                ...
                View mainPart = left.findViewWithTag("MAIN_PART");
                View borderPart = left.findViewWithTag("BORDER_PART");
//                mainPart.setAlpha(1-positionOffset*0.2f);
//                left.setTranslationY(left.getMeasuredHeight()*positionOffset);
                mainPart.setPivotX(mainPart.getMeasuredWidth()*0);
                mainPart.setPivotY(mainPart.getMeasuredHeight()*1.25f);
                mainPart.setTranslationX(positionOffsetPixels);
                mainPart.setCameraDistance(10000f);
//                if(positionOffset==1) mainPart.setVisibility(View.GONE);
//                else mainPart.setVisibility(View.VISIBLE);
//                ViewHelper.setPivotX(left, left.getMeasuredWidth()*positionOffset*0);
//                ViewHelper.setPivotY(left, left.getMeasuredHeight()*positionOffset);
                ViewHelper.setRotationY(mainPart, -120*positionOffset);
//                borderPart.setTranslationX(positionOffsetPixels);
//                if(positionOffset!=1) borderPart.setVisibility(View.INVISIBLE);
//                else borderPart.setVisibility(View.VISIBLE);
//                left.bringToFront();
            }
            if (right != null) {
                manageLayer(right, true);
                right.setTranslationX(-right.getMeasuredWidth()+positionOffsetPixels);
                left.bringToFront();
        ...
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                if (mRot > 90.0f) {
                    ...
                    ViewHelper.setPivotX(left, left.getMeasuredWidth() * 0);
                    ViewHelper.setScaleX(left, 1 - positionOffset);
                    ViewHelper.setPivotY(left, left.getMeasuredHeight() * 0.5f);
                    ViewHelper.setTranslationX(left, mTrans);
                    ViewHelper.setRotationY(left, mRot);
            ...
            if (right != null) {
                ...
                if (mRot < -90.0f) {
                    ...
                    ViewHelper.setPivotX(right, right.getMeasuredWidth() * 0f);
                    ViewHelper.setPivotY(right, right.getMeasuredHeight() * 1f);
                    ViewHelper.setTranslationX(right, mTrans);
                    ViewHelper.setRotationY(right, mRot);
                }
            }
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        switch (mEffect) {
            ...
            case RotateDown:
                ...
                break;
            case Accordion:
                animateAccordion(mLeft, mRight, effectOffset, positionOffsetPixels);
                break;
        }
